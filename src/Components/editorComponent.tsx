                ...
                <div class="rounded-lg border bg-white shadow-sm">
                    ...
                    <div class="p-6">
                        <form hx-post="/api/items" hx-trigger="submit" hx-on--after-request="htmx.trigger(document.body, 'refreshMenuList'); this.reset();" hx-target="#toast-container" class="space-y-6">
                            ...
                            <div>
                                <label class="text-sm font-semibold leading-none">Description</label>
                                <textarea required name="description" class="flex min-h-28 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm " />
                            </div>
                            <div>
                                <label class="text-sm font-semibold leading-none">Price</label>
                                <input required name="price" type="number" step="0.1" class="flex h-10 w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm " />
                            </div>
                            <div>
                                <label class="text-sm font-semibold leading-none">Category</label>
                                <div hx-get="/api/categories" hx-swap="innerHTML" hx-trigger="load, refreshOptions from:body" hx-target="#options">
                                    <div id="options" />
                                </div>
                            </div>
                            <div>
        ...
        <div class="container mx-auto p-4 bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen">
            <div class="rounded-lg border bg-white w-full shadow-lg border-t-4 border-blue-500">
                <div class="flex flex-col space-y-1.5 p-6 bg-white rounded-t-lg">
                    <h3 class="whitespace-nowrap mx-auto tracking-tight text-3xl font-bold text-blue-700">Menu Editor</h3>
                </div>
